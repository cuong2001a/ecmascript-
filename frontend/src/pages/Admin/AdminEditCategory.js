...
const AdminEditCategory = {
    async render() {
        ...
        <div class="w-full overflow-x-hidden border-t flex flex-col">
            <main class="w-full flex-grow p-6">
                ...
                <h1 class="text-3xl text-black pb-6">Sửa danh mục</h1>  
                ...
                <div class="flex space-x-5 my-3 ">
                    ...
                    <input type="text" name=""  id="name" value="${category.name}" placeholder="Tên danh mục" class="border p-2  w-1/2">
                </div>
                <img src="${category.images}"/>
                <input type="file" name="" id="image"  class="border p-2 w-full mt-3">
                <input type="submit" value="Sửa danh mục" id="submit" class="w-full mt-6 bg-blue-600 hover:bg-blue-500 text-white font-semibold p-3">
    ...
    async afterRender() {
        ...
        $('#form-add').addEventListener('submit', (e) => {
            e.preventDefault();
            const categoryImage = $('#image').files[0];
            let storageRef = firebase.storage().ref(`categories/${categoryImage.name}`);
            console.log(storageRef)
            storageRef.put(categoryImage, metadata).then(function () {
                ...
                storageRef.getDownloadURL().then(async (url) => {
                    ...
                    await categoryAPI.update(id, newcategory,_id);
                    window.location.hash = '/listcategories'
